  ...
  return (
    <section ref={container} id="servicos" className="relative py-20 md:py-32 bg-[#C5A059] text-[#121110] transition-colors duration-1000 overflow-hidden">
      <div className="relative z-10 w-full px-6 md:px-[60px] max-w-[1800px] mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-12 mb-16 panel-item opacity-0">
          <div>
        ...
        {/* Expanding Panels - Refined for Touch */}
        <div className="w-full flex flex-col lg:flex-row lg:h-[600px] border-t border-[#121110]/10 mt-8">
          {services.map((service, i) => {
            const isOpened = activeService === i;
            return (
              <div 
                key={i} 
                onClick={() => setActiveService(isOpened ? null : i)}
                className={`panel-item group relative flex-1 transition-all duration-700 ease-[cubic-bezier(0.76,0,0.24,1)] lg:hover:flex-[2.5] border-b lg:border-b-0 lg:border-r border-[#121110]/10 last:border-0 flex overflow-hidden cursor-pointer bg-transparent hover:bg-[#121110] hover:text-[#C5A059] text-[#121110] opacity-0 ${isOpened ? 'lg:flex-[2.5] bg-[#121110] text-[#C5A059]' : ''}`}
                style={{ backgroundColor: isOpened ? '#121110' : undefined }}
              >
                
                {/* Content Inner Box */}
                <div className="p-6 md:p-10 flex flex-col justify-between w-full">
                  
                  {/* Top Number & Expand Icon */}
                  <div className="flex justify-between items-start mb-12">
                    <span className={`font-mono text-[14px] md:text-[16px] opacity-70 transition-colors duration-500 ${isOpened ? 'text-[#EFECE6]' : 'group-hover:text-[#EFECE6]'}`}>
                      // {service.tag}
                    </span>
                    <div className={`w-10 h-10 border border-current opacity-20 rounded-full flex items-center justify-center transition-all duration-500 ${isOpened ? 'rotate-45 opacity-100 border-[#C5A059]' : 'group-hover:opacity-100 group-hover:border-[#C5A059]'}`}>
                      {isOpened ? <Plus size={20} strokeWidth={1.5} /> : <Plus size={20} strokeWidth={1.5} className="group-hover:rotate-45" />}
                    </div>
                  </div>

                  {/* Bottom Title & Revealing Text */}
                  <div className="mt-auto">
                    <h3 className={`font-serif text-[28px] md:text-[42px] tracking-[-0.02em] leading-[1.1] transition-all duration-500 ${isOpened ? 'mb-6 text-[#EFECE6]' : 'group-hover:mb-6 group-hover:text-[#EFECE6]'}`}>
                      {service.title}
                    </h3>
                    
                    {/* Collapsible content block */}
                    <div 
                      className={`grid transition-[grid-template-rows] duration-700 ease-[cubic-bezier(0.76,0,0.24,1)] lg:group-hover:grid-rows-[1fr] ${isOpened ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}
                    >
                      <div className="overflow-hidden">
                        <p className={`font-light text-[15px] md:text-[17px] opacity-80 leading-[1.7] max-w-[350px] pb-8 transition-opacity duration-700 ${isOpened ? 'opacity-100' : 'opacity-0 lg:group-hover:opacity-100'}`}>
                          {service.desc}
                        </p>
                      </div>
                    </div>
